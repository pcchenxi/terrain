...
public:
	...
	Filter_Continuity(int num_one_set, float smooth_size, float cell_size);
	int m_point_num_h;
	float m_smooth_size;
	float m_cell_size;
...
Filter_Continuity::Filter_Continuity(int num_one_set, float smooth_size, float cell_size)
{
	max_continuity = 0;
	m_point_num_h = num_one_set;
	m_smooth_size = smooth_size;
...
float Filter_Continuity::get_varience(Feature *feature_set, int start_index, int size)
{
	...
	if (start < 0)
		start = 0;
	if (end >= m_point_num_h)
		end = m_point_num_h - 1;

	//cout << "start: " << start << "  end: " << end << endl;
	for (int i = start; i < end; i++)
	{
		int index = i % m_point_num_h;
